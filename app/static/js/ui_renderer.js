const ResultRenderer = {
  ...
  renderACV(results, container) {
    ...
    batch.forEach(res => {
      ...
      const faultyCars = (res.cars || []).filter(c => c.severity && c.severity !== 'NORMAL' && c.severity !== 'NO DATA');
      
      if (faultyCars.length > 0) {
        carListHtml += `
          <div style="margin-bottom: 10px;">
            <div style="font-size: 0.85rem; color: var(--text-muted); margin-bottom: 4px;">Faulty Car(s) Detected:</div>
            <div style="display: flex; gap: 8px; flex-wrap: wrap;">
        `;
        faultyCars.forEach(fc => {
          const isCritical = fc.severity === 'CRITICAL';
          const bg = isCritical ? 'rgba(255,51,102,0.1)' : 'rgba(255,153,0,0.1)';
          const border = isCritical ? '#ff3366' : '#ff9900';
          const color = isCritical ? '#ff3366' : '#ff9900';
          
          carListHtml += `
            <div style="font-size: 1.1rem; font-weight: bold; color: ${color}; background: ${bg}; padding: 6px 12px; border-radius: 6px; border: 1px solid ${border};">
              CAR ${fc.car} <span style="font-size: 0.75rem; text-transform: uppercase;">(${fc.severity})</span>
            </div>
          `;
        });
        carListHtml += `</div></div>`;
      } else {
        carListHtml += `<div style="margin-bottom: 10px; color: var(--accent-emerald); font-weight: bold;">✔ Normal - No faulty car detected.</div>`;
      }
      ...
      if (res.ranked_cars) {
        ...
        if (carsArray.length > 0) {
            ...
            carsArray.forEach((car, i) => {
              const isFaulty = faultyCars.some(fc => fc.car === car);
              const bg = isFaulty ? 'rgba(255,51,102,0.2)' : 'rgba(255,255,255,0.05)';
              const col = isFaulty ? '#ff3366' : '#ccc';
